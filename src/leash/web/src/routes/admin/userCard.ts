...
export interface UserInfo {
  ...
  export async function initalizeUserInfo(user: User): Promise<UserInfo> {
    let trainings = await user.getTrainings();
    let updates = await user.getUserUpdates();
    let userCache = new Map<number, User>();
    
    let timelineItems: UserTimelineItem[] = [];

    for (const training of trainings) {
        timelineItems.push(UserTimelineItem.fromTrainingItem({
    ...
    for (const update of updates) {
      timelineItems.push(UserTimelineItem.fromUserUpdate({
    ...
    timelineItems.push(UserTimelineItem.fromUserEvent({
