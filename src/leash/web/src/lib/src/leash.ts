...
async function leashFetch<T extends object>(endpoint: string, method: string, body?: object, noReponse=false): Promise<T> {
    ...
    return await (r.status !== 200 ? Promise.reject(new Error(await r.text())) : noReponse ? r.text() : r.json());
}

export async function searchUsers(query: string, limit=30, offset=0, only_enabled=false, with_training=true, with_updates=true): Promise<SearchUsersResponse> {
    const result = await leashFetch<LeashSearchUsersResponse>(`/api/users/search?q=${query}&allow_empty_body=true&only_enabled=${only_enabled}&with_trainings=${with_training}&with_updates=${with_updates}&limit=${limit}&offset=${offset}`, "GET");
...
export async function getUserByEmail(email: string, with_training=true, with_updates=true): Promise<User> {
    const result = await leashFetch<LeashUser>(`/api/users?email=${email}&with_trainings=${with_training}&with_updates=${with_updates}`, "GET");

    return new User(result);
}

export async function getUserById(id: number, with_training=true, with_updates=true): Promise<User> {
    const result = await leashFetch<LeashUser>(`/api/users?id=${id}&with_trainings=${with_training}&with_updates=${with_updates}`, "GET");
...
export async function createTraining(email: string, training_type: string): Promise<void> {
    await leashFetch(`/api/training`, "POST", {
        email,
        training_type,
    }, true);
}
...
export async function removeTraining(email: string, training_type: string): Promise<void> {
    await leashFetch(`/api/training`, "DELETE", {
        email,
        training_type,
    }, true);
}
