...
<Grid cols={9}>
	<Grid.Col md={6} lg={3}>
		...
		<Stack>
			...
			<Space h="lg" />
			<Button color="blue" fullSize href={'/admin/user?id=' + userInfo.user.id}>Edit User</Button>
		</Stack>
	</Grid.Col>
			...
			{#each userInfo.timelineItems as item}
				<Timeline.Item title={item.getTitle()}>
					<svelte:fragment slot="bullet">
						<ThemeIcon radius="xl" color={item.getBulletColor()}
							><svelte:component this={item.getBullet()} />
						</ThemeIcon>
					</svelte:fragment>
	...
	<Grid.Col md={6} lg={3}>
		...
		<Stack>
			<Modal
				opened={removeTrainingModal.open}
				title="Remove Training"
				centered
				on:close={closeRemoveTrainingModal}
			>
				Are you sure you want to remove {removeTrainingModal.active?.trainingType} from {userInfo
					.user.name}?
				<Space h="lg" />
				<Group position="center">
					<Button on:click={closeRemoveTrainingModal}>Cancel</Button>
					<Space w="lg" />
					<Button
						color="red"
						on:click={async () => {
			...
			<Modal
				opened={createTrainingModal.open}
				title="Create New Training"
				centered
				on:close={closeCreateTrainingModal}
			>
				...
				<Space h="lg" />
				<Group position="center">
					<Button on:click={closeCreateTrainingModal}>Cancel</Button>
					<Space w="lg" />
					<Button
						color="green"
						on:click={async () => {
