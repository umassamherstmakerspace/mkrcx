    ...
    with open(file_name, 'r') as file:
        ...
        for row in reader:
            ...
            if row[DOCUSIGN] == 'TRUE':
                ...
                docusign_training = {
                    "training_type": "docusign"
                }
                response = requests.post(f'{ENDPOINT}/{ID}/trainings', json=docusign_training, headers=headers)
                if response.status_code == 200:
                ...
                response = requests.post(f'{ENDPOINT}/{ID}/holds', json=docusign_hold, headers=headers)
                if response.status_code == 200:
            ...
            if row[ORIENTATION] == 'TRUE':
                ...
                response = requests.post(f'{ENDPOINT}/{ID}/trainings', json=orientation_training, headers=headers)
                if response.status_code == 200:
                ...
                response = requests.post(f'{ENDPOINT}/{ID}/holds', json=orientation_hold, headers=headers)
                if response.status_code == 200:
                    print(f'User {row[EMAIL_INDEX]} orientation hold updated')
                else:
                    print(f'Error updating orientation hold for user {row[EMAIL_INDEX]}')
                    print(response.status_code)
                    print(response.text)
                    continue
